...
export interface PlanReportData {
  ...
  matches?: MatchResult[];
  path?: BuildPath;
  customTarget?: boolean; // path is toward a user-chosen build, not the #1 match
  guide?: SurvivalGuide;
  generatedAt?: string;
}
...
function matchesSection(matches: MatchResult[]): string {
  const rows = matches
    .map((r, i) => {
      const t = r.target;
      const lvl = t.level > 0 ? `L${esc(t.level)}` : "endgame";
      const link = t.sourceUrl ? `<a href="${esc(t.sourceUrl)}" target="_blank">guide ↗</a>` : "";
      return `<li><span class="rank">#${i + 1}</span><span class="pct">${Math.round(r.score * 100)}%</span>
        <div><div class="tname">${esc(t.ascendancy || t.className || "?")} / ${esc(t.mainSkill || "?")} <span class="muted">${lvl}</span> ${link}</div>
        <div class="why">${esc(r.reasons.join("; ") || "weak match")}</div></div></li>`;
    })
    .join("");
  return `<section><h2>Closest builds</h2><ol class="matches">${rows}</ol></section>`;
}

function pathSection(p: BuildPath, customTarget = false): string {
  const t = p.target;
  ...
  return `<section><h2>${customTarget ? "Path to your chosen build" : "Path to the top match"}</h2>
    <div class="target">
      <div class="match">${customTarget ? "→" : Math.round(p.matchScore * 100) + "%"}</div>
      <div>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .link a { color:#6fa8d6; }
  .matches { list-style:none; padding-left:0; margin:0; }
  .matches li { display:flex; gap:12px; align-items:center; padding:8px 10px; background:#23262f; border:1px solid #30343f; border-radius:8px; margin:6px 0; }
  .matches .rank { color:#7c776c; font-size:12px; width:22px; }
  .matches .pct { color:#caa85a; font-weight:700; font-size:18px; width:46px; text-align:right; }
  .matches .tname { font-size:14px; }
  .matches a { color:#6fa8d6; font-size:11px; }
  ...
  ${data.matches && data.matches.length ? matchesSection(data.matches) : ""}
  ${data.path ? pathSection(data.path, data.customTarget) : ""}
